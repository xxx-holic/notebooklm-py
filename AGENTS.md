...
Follow the existing commit style: `feat(cli): ...`, `fix(cli): ...`, `refactor(test): ...`, `style: ...`. PRs should include a short summary, linked issue when relevant, and the commands run locally. For Codex or other parallel agents, prefer `--json`, pass full explicit notebook UUIDs instead of relying on `notebooklm use`, and isolate runs with `NOTEBOOKLM_HOME=<workspace>/.codex/notebooklm/<agent-id>` or `NOTEBOOKLM_PROFILE=codex-<agent-id>` when multiple agents share one machine. The CLI supports `notebooklm skill install --target codex` for `~/.codex/skills/notebooklm/SKILL.md`, and `notebooklm agent show codex` prints the packaged Codex operator guide.
